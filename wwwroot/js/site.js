...
const POLTRONAS = 240;          //constante com o número de poltronas
...
function montarPalco() {
    ...
    if (localStorage.getItem("teatroOcupadas")) {
        //preenche o vetor com as poltronas ocupadas do teatro(salva em localStorage)
    ...
    for (var i = 1; i <= POLTRONAS; i++) {
        ...
        if (i < 10) {               //condições para exibir zeros na poltronas 
        ...
        divPalco.appendChild(figure);               //indica que figura é filha de divPalco

        if (i % 24 == 0) {                              //se módulo 24, restar 0
            var br = document.createElement("br");      //cria tag (quebra de linha)
...
function reservarPoltrona() {
    ...
    //se há poltronas salva em localStorage
    if (localStorage.getItem("teatroOcupadas")) {
        //preenche o vetor com as poltronas ocupadas do teatro (salva em localStorage)
        ocupadas = localStorage.getItem("teatroOcupadas").split(";")
    }

    //se poltrona escolhida ja está ocupada (existe em localStorage)
...
function confirmarReserva() {
    if (reservadas.length == 0) {
        ...
        inPoltrona.focus();                     //elemento ja foi referenciado fe forma global
        return;
    }
    ...
    for (var i = 0; i < reservadas.length; i++) {
        ...
        //captura imagem da poltrona, filha de divPalco. É -1 pois começa com 0
        var imgPoltrona = divPalco.getElementsByTagName("img")[reservadas[i] - 1];

        imgPoltrona.src = "img/ocupada.jpg";            //modifca da imagem
    }

    reservadas = [];        //limpa vetor (pois as reservas ja foram salvas em localStorage)

    // .length-1 é para retirar o ultimo ";"
